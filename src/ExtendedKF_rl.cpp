...
void ExtendedKF_rl::UpdateEKF(const VectorXd &z) {
  ...
  // It gets the position measurement predicted before and converts it to polar 
  // coordinated in order to be able to compare it against the real radar measurement
  float px = x_[0];
  float py = x_[1];
  float vx = x_[2];
  float vy = x_[3];
